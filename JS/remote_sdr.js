...
// ***********************************
const Version="Remote SDR V2.1<br><a href='http://f1atb.fr' target='_blank'>F1ATB</a> April 2021";				
// Variables Audio
//****************
...
les_modes.push([0,"LSB","hack_ssb","pluto_ssbnvl",3800]);
les_modes.push([1,"USB","hack_ssb","pluto_ssbnvl",3800]);
les_modes.push([2,"AM","hack_am","pluto_amnvl",7500]);
les_modes.push([3,"NBFM","hack_nbfm","pluto_nbfmnvl",10000]);
les_modes.push([4,"WBFM","hack_wbfm","pluto_wbfmnvl",150000]);
var timer_info=0;
//Filtres Audio
...
function Lance_Websocket_spectre(){
	...
	websocket_spectre.onmessage = function(evt) {
		if(!web_socket.spectre_in) { //First messages arrive. GNU RADIO launched
			...
			setTimeout("init_para_sdrRX();", 100);
		}
	...
	SDR_RX.mode = $("input[name='mode']:checked").val();
		for (var i=0;i<les_modes.length;i++){
			if (les_modes[i][0]==SDR_RX.mode){
				...
				//var choix_python_script = "pluto_ssbabcd";

				if (SDR_RX.sdr=="pluto") {choix_python_script=les_modes[i][3];} //Pluto SDR

				console.log("gewählter: "+choix_python_script);
				console.log("nvl:"+rx_python_gnuradio_script);
				
				if (choix_python_script!=rx_python_gnuradio_script) {
					...
					console.log("nvl-3");
					var aa=rx_python_gnuradio_script;
					//var loader='<iframe src="' + adresse + '?' + rx_python_gnuradio_script + '" onload="Sockets_launcher();"></iframe>'
					var loader='<iframe src="' + adresse + '?' + aa + '" onload="Sockets_launcher();"></iframe>'
					console.log("nvl-4");
					console.log("send message to client");
					console.log("--nvl-sel:"+adresse+"?"+rx_python_gnuradio_script);
					console.log("***")
...
function Sockets_launcher(){
	console.log("RX lauched: " + rx_python_gnuradio_script)
...
function choix_mode(){ //MODE 
	...
	setTimeout("console.log('Rx call TX mode');Mode_TX();", 800); //Required for Pluto to delay the TX launching
}
...
//Surveillance echange data et timer_info
...
function WatchDog(){
	...
	if(SDR_TX.IP.length>3) {
		Watch_dog.TXpara++;
		if (Watch_dog.TXpara>8 && tx_python_gnuradio_script != "stop") {$("#TXonLed").css("background-color","Red");console.log("TX message not received");} //Alerte messages n'arrivent pas
	}
	if (timer_info>0) {
		timer_info--;
	} else {
